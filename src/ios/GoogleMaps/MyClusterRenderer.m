...
@implementation MyClusterRenderer {
  ...
  // are we animating clusters?
  BOOL _animatesClusters;
...
- (GMSMarker *)markerWithPosition:(CLLocationCoordinate2D)position
  ...
  GMSMarker *marker;
  if ([userData isKindOfClass:[GMSMarker class]]) {
    // let's use the original marker!
    marker = userData;
  } else {
    marker = [self markerForObject:userData];
